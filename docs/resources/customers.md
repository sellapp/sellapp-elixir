...
```elixir
def create_customer(client, params \\ %{}, opts \\ [])
```
...
```elixir
def update_customer(client, customer, params \\ %{}, opts \\ [])
```
...
```elixir
def get_customer_by_external_id(client, external_id, opts \\ [])
```
...
```elixir
def upsert_by_external_id(client, external_id, params \\ %{}, opts \\ [])
```
...
{:ok, result} = SellApp.Customers.upsert_by_external_id(client, "crm_maya_314", %{"email" => "maya.chen@example.com", "name" => "Maya Chen"})
IO.inspect(result)
```
...
```elixir
def update_customer_by_external_id(client, external_id, params \\ %{}, opts \\ [])
```
