...
```elixir
def create(client, affiliate, opts \\ [])
```
...
{:ok, result} = SellApp.AffiliatePayouts.create(client, 1, idempotency_key: "example-mutation-001")
IO.inspect(result)
```
...
```elixir
def update(client, payout, params \\ %{}, opts \\ [])
```
