...
```elixir
def add(client, group, params \\ %{}, opts \\ [])
```
...
```elixir
def remove(client, group, params \\ %{}, opts \\ [])
```
...
```elixir
def list(client, group, params \\ %{}, opts \\ [])
```
...
```elixir
def replace(client, group, params \\ %{}, opts \\ [])
```
...
{:ok, result} = SellApp.GroupsProducts.replace(client, 42, %{"product_ids" => [120, 121]})
IO.inspect(result)
```
...
```elixir
def get(client, group, product, opts \\ [])
```
...
```elixir
def search(client, group, params \\ %{}, opts \\ [])
```
...
{:ok, result} = SellApp.GroupsProducts.search(client, 1, %{"filters" => [%{"field" => "id", "operator" => "=", "value" => 1}], "sort" => [%{"field" => "created_at", "direction" => "desc"}]})
IO.inspect(result)
```
