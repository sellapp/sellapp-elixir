...
```elixir
def list_refunds(client, params \\ %{}, opts \\ [])
```
...
{:ok, result} = SellApp.Refunds.list_refunds(client, %{})
IO.inspect(result)
```
...
```elixir
def get_refund(client, refund, opts \\ [])
```
...
{:ok, result} = SellApp.Refunds.get_refund(client, 781)
IO.inspect(result)
```
