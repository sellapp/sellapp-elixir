# This file is auto-generated by oagen. Do not edit.

defmodule SellApp.ListExportsResponseValue200ApplicationJsonPropertyDataItemFormat do
  @moduledoc """
  ListExportsResponseValue200ApplicationJsonPropertyDataItemFormat enum.
  """

  @type t :: :csv | :json

  @doc "All known values."
  @spec values() :: [t()]
  def values, do: [:csv, :json]

  @doc "Casts a wire string to its atom; unknown values pass through unchanged."
  @spec cast(String.t() | nil) :: t() | String.t() | nil
  def cast("csv"), do: :csv
  def cast("json"), do: :json
  def cast(other), do: other

  @doc "Dumps an atom back to its wire string."
  @spec dump(t() | String.t()) :: String.t()
  def dump(:csv), do: "csv"
  def dump(:json), do: "json"
  def dump(other) when is_binary(other), do: other
end
